...
extern TYPE *make_array_type	(TYPE* idx,TYPE *elt); //Iris add (F3)
extern TYPE *make_record_type(FIELDLIST* fi); //Iris add (F3)
...
extern STMTFOR    *make_stmtfor    (BINDING *b, EXPR *test, BOOLEAN up, EXPR *test2, STMT *body); //Iris add (F10)
...
extern STMT *make_forstmt      (IDENT *id, EXPR *test, BOOLEAN up, EXPR *test2, STMT *body); //Iris add (F10)
