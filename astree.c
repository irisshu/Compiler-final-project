...
//Iris add (Feature 5)
PROCFUNCDEF *make_procfundef (DECLLIST *dl, TYPE *t, BLOCK *b)
{
...
DECL *make_procfunc_decl (IDENT *id, DECLLIST *dl, IDENT *ret, BLOCK *b)
{
	...
	d->d.proc = make_procfundef (dl, t, b); 	
	return d;
}
...
//Iris add
TYPE *make_record_type(FIELDLIST *fi)
{
    TYPE *t  = make_type(TypeRecord_);
	t->t.rec = fi; 
	// struct FieldList *rec; 因為FieldList已經是rec，所以直接傳就好.
	return t;
}
...
FIELD *make_field (IDENT *id, TYPE *t) //Iris add (F4)
{
  FIELD *f = anew (FIELD);
  f->field_id   = id;
  f->field_type = t;
  f->field_offset = yylineno;
  return f;
}

FIELDLIST *make_fieldlist (FIELD *f, FIELDLIST *fl) //Iris add (F4)
{
